...
window.addEventListener('load', () => {
    ...
    mapa = datos.mapa;
    miId = datos.tuId;
    // Convertir jugadores a objeto mutable correctamente
    jugadores = {};
    if (datos.jugadores) {
        Object.entries(datos.jugadores).forEach(([id, j]) => {
            jugadores[id] = { ...j };
        });
    }
    yo = jugadores[miId];
    if (!yo) {
        // Si no está, crearlo
        yo = { x: 100, y: 100, angle: 0, hp: 100, maxHp: 100, vivo: true, kills: 0, muertes: 0, monedas: 0, nombre: 'Tú' };
        jugadores[miId] = yo;
    }
    monedas = {};
    ...
    // Confirmar sala al servidor después de que el socket conecte
    setTimeout(() => {
        if (socket) socket.emit('confirmar_sala', datos.salaId);
    }, 500);
});
...
function conectarSocket(token, salaId) {
    ...
    socket.on('jugador_movio', ({ id, x, y, angle, arma }) => {
        if (id === miId) return; // No actualizar nuestra propia posición
        if (!jugadores[id]) {
            jugadores[id] = { nombre: id.startsWith('npc_') ? 'NPC' : id, vivo: true, hp: 100, maxHp: 100, esNPC: id.startsWith('npc_'), skin: 'jaguar' };
        }
        jugadores[id].x = x;
        jugadores[id].y = y;
        jugadores[id].angle = angle;
        if (arma !== undefined) jugadores[id].arma = arma;
    });
    ...
    socket.on('sync_jugadores', (jgds) => {
        Object.entries(jgds).forEach(([id, j]) => {
            if (id === miId) return;
            jugadores[id] = { ...j };
        });
    });
...
function procesarInput(dt) {
    ...
    const m=16;
    if (!colisiona(nx, yo.y, m)) { yo.x=nx; jugadores[miId].x=nx; }
    if (!colisiona(yo.x, ny, m)) { yo.y=ny; jugadores[miId].y=ny; }
    ...
    // Enviar posición al servidor (20fps = cada 50ms)
